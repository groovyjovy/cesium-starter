...
url = "http://localhost:3000/geojsons"

def post_single_geojson(file_path, url):
    """
    個別のXMLファイルを送信する関数
    """
    data = {
...
def post_jsons_concurrently(url, max_workers=1):
    """
    XMLファイルを並列に送信する関数
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {executor.submit(post_single_geojson, file, url): file for file in xml_files}
        for future in as_completed(future_to_file):
            print(future.result())

# 実行
post_jsons_concurrently(url)
